...
After setting $\mathbb{E}_{r(y \mid f_n)}[]$ as an constant and setting $f_n$ as
$\psi(x)$ (right) and $\psi^*(x)$ (left), the FIM will exactly become the S matrix.
It's easy to set the expectation part as an constant by using Gaussian distribution
(`register_normal_predictive_distribution` provided by kfac_jax). However, setting
$f_n$ as the complex wavefunction is a little bit tricky, as kfac_jax is written for
...
def vjp_rc_with_aux(fun: Callable, *primals, has_aux=True) -> tuple[Any, Callable, Any]:
    """Compute the VJP product for a function with real inputs and complex outputs.
...
def patched_layer_tag_vjp(processed_jaxpr, primal_func_args):
    """Monkey patching `jax.vjp` with the one supporting R->C."""
    jax.vjp = vjp_rc_with_aux  # type: ignore
    try:
...
class PatchedBlockDiagonalCurvature(BlockDiagonalCurvature):
    ...
    def __init__(self, func, *args, **kwargs):
        ...
        self._vjp, self._jaxpr_extractor = cached_transformation(
            func=func,
